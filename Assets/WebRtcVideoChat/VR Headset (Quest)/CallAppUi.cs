...
public class CallAppUi : MonoBehaviour
{
    ...
    /// <summary>
    /// Texture of the remote video for the skybox
    /// </summary>
    protected Texture2D mRemoteSkyBoxVideoTexture = null;
    ...
    public virtual void UpdateLocalTexture(ProcessedFrame frame)
    {
        if (uLocalVideoImage != null)
        {
            if (frame != null)
            {
                if (String.IsNullOrEmpty(frame.material) == false)
                {
                    Debug.LogWarning("FrameProcessor returned a texture that requires a specific material."
                        + " The UI is not prepared for this!");
                }
                uLocalVideoImage.texture = frame.texture;

                //enable in case it was disabled before
                ...
                mHasLocalVideo = true;
                mLocalFrameCounter++;
                mLocalVideoWidth = frame.MetaData.Width;
                mLocalVideoHeight = frame.MetaData.Height;
                mLocalVideoFormat = frame.MetaData.SourceFormat;
                mLocalRotation = frame.MetaData.Rotation;
            }
            else
            {
                //app shutdown. reset values
                mHasLocalVideo = false;
                if (uLocalVideoImage.texture != null && uLocalVideoImage.texture != this.uNoCameraTexture)
                {
    ...
    /// </summary>
    /// <param name="frame"></param>
    public virtual void UpdateRemoteTexture(ProcessedFrame frame)
    {
        if (uRemoteVideoImage != null)
        {
            if (frame != null)
            {
                ...
                mRemoteVideoWidth = frame.MetaData.Width;
                mRemoteVideoHeight = frame.MetaData.Height;
                mRemoteVideoFormat = frame.MetaData.SourceFormat;
                mRemoteRotation = frame.MetaData.Rotation;
                //SkySphere code. Used skysphere as opposed to skybox as video stream comes through upside down and is easier to manipuilate a sphere as opposed to a skybox. 
                skyboxMaterial.mainTexture = uRemoteVideoImage.texture;
                mRemoteFrameCounter++;
            }
            else
            {
    ...
    /// <summary>
    /// Triggers all UI side configuration to be stored and applied to the CallApp
    /// </summary>
    public void Apply()
    {
